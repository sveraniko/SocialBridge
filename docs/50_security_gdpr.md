...
### 7.1 Dynamic mappings retention note
Динамические маппинги fallback (`result=fallback_payload`) сохраняются в `sb_content_map` с `channel=generic`, `content_ref=dyn:*`, `meta.dynamic=true`.

Рекомендация для ops:
- контролировать суточный рост и периодически чистить устаревшие записи вручную (SQL/ops-скрипт),
- перед удалением сверять, что запись давно не используется и не нужна активным кампаниям.
